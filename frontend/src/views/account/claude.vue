...
import {
  ...
  GetClaudeState,
  QuickImportClaudeAccount,
  RefreshAllClaudeUsage,
  SaveClaudeUIState,
...
const addAccountSteps = computed(() => [
  t("claude.account.step0"),
  t("claude.account.step1"),
  t("claude.account.step2"),
  t("claude.account.step3"),
  t("claude.account.step4"),
]);
...
async function detectCurrent() {
  ...
  message.value = "";
  try {
    const fresh = await GetClaudeState();
    appState.value = fresh;
    syncSelectedProfile(fresh);
    message.value = t("claude.account.detectSuccess");
  } catch (err) {
    error.value = normalizeError(err);
  } finally {
...
async function quickImport() {
  if (activeMissing.value) {
    ...
    if (activeMissing.value) {
      error.value = t("claude.account.noCurrentAccount");
      return;
    }
  }
  if (savedActiveProfile.value) {
    selectedProfile.value = savedActiveProfile.value.id;
    await SaveClaudeUIState(savedActiveProfile.value.id);
    message.value = t("claude.account.alreadySaved");
    return;
  }
  await runAction(async () => {
    const state = await QuickImportClaudeAccount();
    appState.value = state;
    syncSelectedProfile(state);
    message.value = t("claude.account.saveSuccess");
  });
}
...
async function refreshAllProfiles() {
  ...
  message.value = "";
  try {
    const state = await RefreshAllClaudeUsage();
    appState.value = state;
    syncSelectedProfile(state);
    message.value = t("claude.account.refreshAllSuccess");
  } catch (err) {
    error.value = normalizeError(err);
  } finally {
...
async function activateProfile(profile?: main.Profile) {
  ...
  if (!id) {
    error.value = t("claude.account.selectAccount");
    return;
  }
  await runAction(async () => {
    let state: main.AppState;
    try {
      state = await ActivateClaudeProfile(id);
    } catch (err) {
      if (normalizeError(err) === "already active") {
        selectedProfile.value = id;
        await SaveClaudeUIState(id);
        message.value = t("claude.account.alreadyActive");
        return;
      }
      throw err;
    }
    ...
    if (matched && restarted) {
      message.value = t("claude.account.switchSuccess");
    } else if (matched) {
      error.value = t("claude.account.switchRestartFail", { status: state.restartStatus });
    } else {
      error.value = t("claude.account.switchMismatch");
    }
  });
}
...
function usageStatus(usage?: UsageSnapshot | null) {
  if (!usage) return t("claude.account.usageNotLoaded");
  if (usage.status && usage.status !== "ok") return usage.status;
  if (!usage.windows?.length) return t("claude.account.usageNoData");
...
function profilePlanLabel(profile: main.Profile) {
  return profile.usage?.planType || profile.plan || t("claude.account.unknownPlan");
}
    ...
    <section class="relative z-10 flex h-screen w-full flex-col">
      ...
      <div v-if="!loaded" class="skel-wrap flex flex-col gap-2.5 px-3 pb-4 pt-3 sm:gap-3 sm:px-4">
        <div class="flex items-center gap-2 px-0.5 py-0.5">
          ...
          <span class="text-xs text-[var(--app-muted)]">{{ t("claude.account.loadingHint") }}</span>
        </div>
      ...
      >
        <div class="flex items-center justify-between gap-3">
          <div class="flex items-center gap-3">
            <button
              ...
              @click="router.push({ name: 'claude' })"
            >
              <span class="block h-4 w-4 shrink-0 icon-[mdi--arrow-left]"></span>
            </button>
            <h2 class="text-base font-semibold">{{ t("claude.account.pageTitle") }}</h2>
          </div>
        </div>
        ...
        >
          <div class="mb-4 flex min-w-0 items-center justify-between gap-3 [&_h2]:text-base [&_h2]:font-semibold">
            <h2>{{ t("claude.account.currentSection") }}</h2>
            <button
            ...
            >
              ...
              <span>{{ detectBusy ? t("common.loading") : t("claude.account.detectBtn") }}</span>
            </button>
          </div>

          <p class="mb-3 text-xs leading-relaxed text-[var(--app-muted)]">{{ t("claude.account.fileHint") }}</p>
          ...
          >
            <p class="m-0 text-sm font-semibold">{{ active.label || t("claude.account.noAccount") }}</p>
            <p class="mt-2 text-xs leading-relaxed text-[var(--app-muted)]">{{ t("claude.account.authFileLine") }}</p>
          </div>
          ...
          <div class="flex justify-start">
            ...
            >
              ...
              <template v-else>{{ t("claude.account.saveAccount") }}</template>
            </button>
          </div>
        </section>
        ...
        >
          <div class="mb-4 flex min-w-0 items-center justify-between gap-3 [&_h2]:text-base [&_h2]:font-semibold">
            <h2>{{ t("claude.account.stepsTitle") }}</h2>
          </div>

          <p class="mb-4 text-xs leading-relaxed text-[var(--app-muted)]">{{ t("claude.account.stepsNote") }}</p>
        ...
        >
          <div class="mb-4 flex min-w-0 items-center justify-between gap-3 [&_h2]:text-base [&_h2]:font-semibold">
            <h2>{{ t("claude.account.savedSection") }}</h2>
            <div class="flex items-center gap-2">
              <button
                class="no-drag inline-flex h-7 items-center justify-center gap-1 rounded-xl border border-[var(--app-border)] bg-transparent px-2.5 text-xs font-medium text-[var(--app-fg)] outline-none transition-[background,color,opacity] duration-150 hover:bg-[var(--app-hover)]"
                :disabled="busy || usageBusy"
              ...
              >
                ...
                <span>{{ usageBusy ? t("common.refreshing") : t("claude.account.refreshUsage") }}</span>
              </button>
              <span
                class="rounded-full border border-[var(--app-border)] px-2 py-0.5 text-xs font-medium text-[var(--app-muted)]"
                >{{ t("claude.account.countSuffix", { n: profiles.length }) }}</span
              >
            </div>
          </div>
          ...
          >
            <p>{{ t("claude.account.noSaved") }}</p>
            <span>{{ t("claude.account.noSavedHint") }}</span>
          </div>
      ...
      <Transition name="confirm-modal">
        ...
        >
          ...
          >
            <p class="mb-1 text-sm font-semibold">{{ t("claude.account.confirmSwitchTitle") }}</p>
            <p class="mb-5 text-xs leading-relaxed text-[var(--app-muted)]">
              {{ t("claude.account.confirmSwitchBody", { name: confirmProfile.label || confirmProfile.name }) }}
            </p>
            <div class="flex justify-end gap-2 [&_button]:h-[38px] [&_button]:w-[92px] [&_button]:px-0">
              ...
              >
                ...
                <template v-else>{{ t("claude.account.confirmSwitch") }}</template>
              </button>
            </div>
          </div>
        </div>
